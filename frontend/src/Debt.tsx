      ...
      <Card className="mt-6">
        ...
        <Card className="w-full">
          <Table>
            ...
            <TableBody>
              {debts.map((debt) => (
                <TableRow key={debt._id} className="text-center bg-white">
                  ...
                  <TableCell className="px-4 py-2 font-medium text-left">{debt.name}</TableCell>
                  <TableCell className="px-4 py-2 text-left">${isNaN(debt.amount) ? "N/A" : debt.amount.toFixed(2)}</TableCell>
                  <TableCell className="px-4 py-2">
                    <button onClick={() => handleDeleteTransaction(debt._id, "Debt")} className="hover:text-red-500">
                      <TrashBinIcon/>
                    </button>
                  </TableCell>
                </TableRow>
              ...
              {credits.map((credit) => (
                <TableRow key={credit._id} className="text-center bg-white">
                  <TableCell className="px-4 py-2 text-left">
                    <Badge className="bg-green-100 text-green-600 dark:bg-green-900/50 dark:text-green-400" variant="outline">
                      Credit
                    </Badge>
                  </TableCell>
                  <TableCell className="px-4 py-2 font-medium text-left">{credit.name}</TableCell>
                  <TableCell className="px-4 py-2 text-left">${isNaN(credit.amount) ? "N/A" : credit.amount.toFixed(2)}</TableCell>
                  <TableCell className="px-4 py-2">
  ...
  return (
    ...
    >
      <path
        ...
        fillRule="evenodd"
      />
      <path d="M9 9H11V17H9V9Z" fill="#f44336" />
      <path d="M13 9H15V17H13V9Z" fill="#f44336" />
    </svg>
  );
}