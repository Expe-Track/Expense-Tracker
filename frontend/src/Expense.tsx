...
export default function Expense() {
  ...
  const DeleteExpense = async (expenseId: string) => {
    try {
      await axios.delete(`http://localhost:8000/exp/delete/${expenseId}`, {
        headers: {
          Authorization: `Bearer ${
            document.cookie
              .split("; ")
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredExpenses.map((expense, index) => (
                  <tr key={index}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        onClick={() => setExpenseIdToDelete(expense._id)}
              ...
              <div className="fixed inset-0 flex items-center justify-center z-50">
                <div className="bg-white p-6 rounded-md shadow-md">
                  ...
                  <div className="flex justify-end">
                    <button
                      onClick={() => setExpenseIdToDelete("")}
                      className="mr-2 px-4 py-2 bg-gray-200 hover:bg-gray-300 rounded-md"
                    >
                      Cancel
                    </button>
                    <button
                      onClick={() => DeleteExpense(expenseIdToDelete)}
